...
export function AdminDashboard({ claims, navigate, onLogout, notice, setNotice }) {
  ...
  const clearTransactions = async () => {
    if (!window.confirm('Удалить все транзакции? Балансы карт не изменятся.')) return;
    await withFeedback(async () => {
      await api('/admin/clear-transactions', { method: 'POST' });
      await loadTransactions();
    }, 'Все транзакции удалены');
  };

  const resetDemoData = async () => {
    if (!window.confirm('Сбросить demo-БД? Будут удалены карты, терминалы и транзакции. Останется карта 84f07c05 с балансом 500.')) return;
    await withFeedback(async () => {
      await api('/admin/reset-demo-data', { method: 'POST' });
      await refreshAll();
    }, 'Demo-БД сброшена');
  };
    ...
    <PageFrame
      ...
      actions={
        <>
          ...
          <TopButton variant="ghost" onClick={clearTransactions}>
            Очистить транзакции
          </TopButton>
          <TopButton variant="danger" onClick={resetDemoData}>
            Сбросить demo-БД
          </TopButton>
